...
static void scatter(VkyPanel* panel)
{
    ...
    VkyVisual* visual = vky_visual_marker(panel->scene, params, false);
    vky_add_visual_to_panel(visual, panel, VKY_VIEWPORT_INNER, VKY_VISUAL_PRIORITY_NONE);
...
static void imshow(VkyPanel* panel)
{
    ...
    VkyColorBytes* image = calloc(size * size, sizeof(VkyColorBytes));
...
static void graph(VkyPanel* panel)
{
    ...
    VkyVisualBundle* graph = vky_bundle_graph(panel->scene, params);
    vky_add_visual_bundle_to_panel(graph, panel, VKY_VIEWPORT_INNER, VKY_VISUAL_PRIORITY_NONE);
