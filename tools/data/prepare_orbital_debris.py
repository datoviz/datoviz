...
try:
    ...
    ImportError
) as exc:  # pragma: no cover - exercised only without the inline dependency runner.
    raise SystemExit('missing `sgp4`; run `uv run tools/data/prepare_orbital_debris.py`') from exc
...
class DebrisEvent:
    """One CelesTrak debris-event query and display label."""
    ...
    @property
    def url(self) -> str:
        """Return the documented CelesTrak GP CSV query URL."""
...
def _download_event(
    ...
    *,
    ...
    if source_path.exists() and (
        offline or (not refresh_source and _cached_source_is_fresh(metadata_path, now))
    ):
        metadata = json.loads(metadata_path.read_text(encoding='utf8'))
    ...
    request = urllib.request.Request(  # noqa: S310 - fixed HTTPS CelesTrak endpoint.
        event.url,
        headers={
            'Accept': 'text/csv',
    ...
    try:
        with urllib.request.urlopen(  # noqa: S310 - request URL is fixed above.
            request, timeout=120
        ) as response:
        ...
        raise RuntimeError(
            f'CelesTrak request failed; stopping without retry: {exc.reason}'
        ) from exc
...
def _balanced_limit(
    rows_by_event: list[list[dict[str, Any]]], max_objects: int
) -> list[dict[str, Any]]:
...
def _teme_to_visual(
    position_km: tuple[float, float, float], julian_date: float
...
def _write_metadata(
    ...
    write_json(
        path,
        {
            **binary_metadata,
            'coordinate_frame': (
                'TEME positions propagated with SGP4, rotated to approximate '
...
def prepare(args: argparse.Namespace) -> Path:
    """Download, propagate, validate, and write the cache-only prepared bundle."""
    ...
    write_provenance(
        ...
        title='Catalogued Orbital Debris',
        source_lines=[
            'CelesTrak OMM-compatible GP element sets for FENGYUN 1C, '
            'IRIDIUM 33, and COSMOS 2251 debris.',
        ...
        processing_lines=[
            ...
            'Converted TEME positions to approximate Earth-fixed coordinates using GMST '
        ...
        notes=[
            'These are tracked catalog objects from three selected debris events, '
            'not the full debris environment.',
            'The Earth-fixed conversion omits polar motion and other high-precision '
            'Earth-orientation corrections.',
            'Point sizes in the visualization are exaggerated and do not encode '
