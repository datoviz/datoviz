...
def _classify_lane(
    ...
    runtime_churn_resources = {"gpu", "vulkan", "glfw", "video", "global-state"}
    if runtime_churn_resources & resource_set and any(
        keyword in text for keyword in SLOW_CHURN_KEYWORDS
    ):
    ...
    if module in RENDER_MODULES:
        if {"gpu", "vulkan", "glfw"} & resource_set:
            return "render-smoke", "presentation/offscreen render coverage"
        return "fast-cpu", "non-GPU support module coverage"
...
def _filter_cases(cases: list[TestCaseInventory], lane: str | None = None) -> list[TestCaseInventory]:
    if lane is None:
        return cases
    filtered = [case for case in cases if case.lane == lane]
    if not filtered:
        known = ", ".join(sorted({case.lane for case in cases}))
        raise ValueError(f"unknown or empty lane {lane!r}; known lanes: {known}")
    return filtered
...
def build_inventory(
    ...
    cases = _filter_cases(cases, lane)
...
def _write_case_list(path: Path, inventory: TestInventory) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Generated by tools/test_inventory.py.",
        "# Pass to dvztest with: --case-list <path>",
    ]
    lane = inventory.source.get("lane")
    if lane is not None:
        lines.append(f"# lane: {lane}")
    lines.extend(case.case_id for case in inventory.cases)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--lane", help="Restrict output to one computed lane.")
    parser.add_argument(
        "--timing-json",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.case_list is not None:
        _write_case_list(args.case_list, inventory)
