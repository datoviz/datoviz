#!/usr/bin/env python3
"""Smoke-test raw ctypes and the array facade from editable and wheel installs."""
...
assert dvz.dvz_visual_set_data(visual, "position", positions) == 0
dvz.dvz_scene_destroy(scene)
print(f"ctypes package smoke: OK ({impl.dvz._name})")
'''
...
def editable_smoke() -> None:
    ...
    _install_runtime_python_deps(python)
    _run(
        [
            str(python),
            '-m',
...
def _copy_package_root(path: Path) -> None:
    ...
    if not (package_dir / '_array_facade.py').exists():
        raise RuntimeError('datoviz/_array_facade.py is missing; run `just ctypes` first')
