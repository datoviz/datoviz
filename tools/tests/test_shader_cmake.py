...
class ShaderCMakeTest(unittest.TestCase):
    ...
    def test_invalid_environment_glslc_path_fails_immediately(self) -> None:
        """An invalid environment override reports the namespaced setting and exact path."""
        missing = '/definitely/missing/datoviz-env-glslc'
        result = self._run_cmake(env_overrides={'DVZ_GLSLC': missing})
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('Configured DVZ_GLSLC environment path does not exist', result.stderr)
        self.assertIn(missing, result.stderr)

    def test_required_precompiled_shaders_require_glslc(self) -> None:
        """Strict precompilation is diagnosed before shader products are configured."""
        result = self._run_cmake(
            '-DDVZ_REQUIRE_PRECOMPILED_SHADERS=ON',
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('glslc is required by DVZ_REQUIRE_PRECOMPILED_SHADERS=ON', result.stderr)
