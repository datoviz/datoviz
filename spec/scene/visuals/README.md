...
| `glyph` | [GLYPH.md](GLYPH.md) | `dvz_glyph()` plus transitional `dvz_text()` lowering | text/font/annotation state lowers to glyph visuals; semantic `DvzText` API still pending | atlas-backed bitmap/SDF/MSDF-capable glyph path | no | semantic API cleanup, data/world placement, HarfBuzz shaping, diagnostics, glyph/text picking |
