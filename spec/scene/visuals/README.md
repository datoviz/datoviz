...
These documents refine `../semantics/VISUAL_FAMILY_RULES.md` and
`../semantics/VISUAL_CONTRACT.md` with enough concrete detail to implement or validate each family.
...
names above. Examples include `dvz_sphere_set_mode()`, `dvz_visual_set_material()`, shared
...
The canonical per-family implementation snapshot lives in [STATUS.md](STATUS.md). The per-family
specs below are broader than the current implementation.
...
1. [STATUS.md](STATUS.md): current per-family implementation snapshot.
2. [SHARED_ATTRIBUTES.md](SHARED_ATTRIBUTES.md): shared visual attributes and style vocabulary.
3. [IMPLEMENTATION_LAYOUT.md](IMPLEMENTATION_LAYOUT.md): current implementation ownership layout.
