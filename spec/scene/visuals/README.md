...
| `glyph` | [GLYPH.md](GLYPH.md) | `dvz_glyph()` low-level plus current `dvz_text()` lowering | text/font/annotation state lowers to glyph visuals; semantic `DvzText` migration pending | atlas-backed bitmap/SDF/MSDF-capable glyph path | no | semantic API migration, data/world placement, HarfBuzz shaping, diagnostics, glyph/text picking |
