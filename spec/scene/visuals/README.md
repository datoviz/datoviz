...
| `path` | [PATH.md](PATH.md) | `dvz_path()` | line-strip plus optional subpaths/stroke_width/caps/joins | primitive line-strip or path-native stroked lowering | stroke GPU picking over lowered edges | first-class closed-path API, dashes, path/subpath identity picking, WGSL parity |
...
| `image` | [IMAGE.md](IMAGE.md) | `dvz_image()`, field binding, texture wrappers | multi-item position/extent/anchor/tex_rect/tint over 2D `SampledField`, scale/colormap binding, partial updates | textured rectangle path | image item picking and image/segment probe readback | richer probe payloads and tiled/LOD policy |
| `labels` | [IMAGE.md](IMAGE.md), active labels proposal | `dvz_labels()`, field + categorical scale binding | integer 2D `SampledField`, categorical scale, opacity/background/selected/hidden/boundary/fallback style | integer texture fetch with GLSL and WGSL variants | no raw-label GPU probe yet | 3D label slices, GPU label probing, larger sparse-id pressure tests |
