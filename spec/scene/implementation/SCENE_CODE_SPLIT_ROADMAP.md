...
9. `visuals/registry/`: first `DvzVisualFamilyOps` table, active-family coverage test, retained
   ...
   resolution, and runtime bind/pipeline/shader/draw descriptor dispatch. Retained lowering bodies
   now live in active family folders.
...
query, bounds, and pass-capability/bind/pipeline/shader/draw hook bodies into family-owned files
instead of adding more root-level visual switches. See
