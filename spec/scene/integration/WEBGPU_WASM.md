...
2. The browser scene slices create point, pixel, marker, segment, path, primitive, image, mesh,
   sphere, panzoom, camera, and arcball objects through the generic ABI.
...
1. `just wasm-scene-smoke` builds the Emscripten scene ABI, emits
   `build-wasm-scene/wasm/wasm_api_scene_point_pixel_marker_segment_path_primitive_image_mesh_panzoom.json` and
