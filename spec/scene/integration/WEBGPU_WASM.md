...
2. The browser scene slices create point, pixel, marker, primitive, image, mesh, panzoom, camera,
   and arcball objects through the generic ABI.
...
2. from browser JavaScript, create one scene, one figure, one panel, and point, pixel, marker,
   primitive triangle-list, RGBA8 image, and basic mesh visuals;
...
1. `just wasm-scene-smoke` builds the Emscripten scene ABI, emits
   `build-wasm-scene/wasm/wasm_api_scene_point_pixel_marker_primitive_image_mesh_panzoom.json` and
...
The supported browser-scene subset now covers point, pixel, basic marker, primitive triangle-list,
RGBA8 image, basic mesh, panzoom, and a first 3D mesh + arcball scene. The browser wrapper passes normalized WebGPU
...
pixel, and marker updates, 2D image texture resize reloads, and 3D mesh retained updates. The browser
