...
ImGui uses its own font atlas, independent of scene `DvzFont` and `DvzTextAtlas` resources. Its native defaults use embedded Source Sans 3 Regular for UI text and Source Code Pro Regular for monospace text, with bounded Noto Sans Math scientific ranges merged into the regular UI atlas. Configured application font paths may override the sans and monospace faces. Scene and ImGui share immutable embedded font bytes and the same default-family policy, but never share atlas packing, textures, GPU resources, or lifetime. The ownership and default-family contract is specified in [../../data/FONT_ARCHITECTURE.md](../../data/FONT_ARCHITECTURE.md), and the public C font functions are documented in the generated scene and runtime-utilities API references.
