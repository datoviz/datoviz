...
1. Manifest counts: `81 webgpu-live`, with remaining examples explicitly classified as
...
2. Live routes cover basic scene, timer animation, triangulation, builtin shapes 2D/3D, isolines,
   ...
   signed categorical label probe, quickstart scatter plot, and retained UTC datetime axis.
...
3. **Approachable deferred features.** `start_scatter` and `features_datetime_axis` are live with
   automated browser proof. Reassess `features_marker_symbols`; keep native GUI behavior out of
   portable scenarios and complete only the atlas parity needed by the canonical example.
