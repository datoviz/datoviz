...
2. Live routes cover basic scene, timer animation, triangulation, builtin shapes 2D/3D, isolines,
   ...
   signed categorical label probe, quickstart scatter plot, retained UTC datetime axis, and
   texture-backed marker symbol sets.
...
3. **Approachable deferred features.** Complete: `start_scatter`, `features_datetime_axis`, and
   `features_marker_symbols` are live with automated browser proof. Marker bitmap/SDF/MSDF atlas
   rendering uses shared scene/DRP2 contracts and WGSL shader variants.
