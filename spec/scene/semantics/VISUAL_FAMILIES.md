...
| `glyph` | `dvz_glyph()` plus transitional `dvz_text()` lowering | text/font/annotation state lowers to glyph visuals; semantic `DvzText` API still pending | atlas-backed bitmap/SDF/MSDF-capable glyph path through scene/DRP2 | no | semantic text API cleanup, data/world placement, HarfBuzz shaping, diagnostics, and glyph/text picking remain deferred |
