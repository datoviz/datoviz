...
| Terrain/planet | `textured_planet.c`; historical `GRAND_CANYON_FLYOVER.md` / `EARTH.md` notes | v0.4 required showcase slice | Retained textured mesh, UVs, sampler state, lit sphere-like surface, camera/capture. | One deterministic UV-mapped Earth/Mars planet using true mesh texture sampling, not baked vertex colors. Full terrain/DEM workflow follows later. |
