...
| Point visual mapping | `dvz_point()`, dense `position`, `color`, `diameter`, optional `item_state` | `include/datoviz/scene.h`, `src/scene/visuals/point/`, `examples/c/visuals/point.c` | First GSP point slice is straightforward. |
...
| Controller mapping | `dvz_view_panzoom()`, `dvz_view_arcball()`, `dvz_view_turntable()`, `dvz_view_fly()`, `dvz_view_orbit_camera()` | `include/datoviz/app.h`, `src/scene/core/controllers.c`, controller headers | Public app helpers cover expected interaction families. |
...
| Nonlinear transforms/projections | Apply in GSP/producer code before upload for v0.4 | Datoviz DATA/VIEW/PANEL coordinates and matrix transforms |
