...
5. Complete for core documentation: generated gallery pages and the machine-readable example inventory derive media publication intent from the source manifest and no longer inspect a local capture tree; strict hermetic builds use generated-only media stand-ins, while `DATOVIZ_DOCS_SITE_ASSETS=1` explicitly enables real gallery and WebGPU bundle staging for serving and publication. Moving those publication inputs from legacy `data` paths to catalog-pinned assets remains Phase 4 work.
