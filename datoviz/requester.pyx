...
_ACTIONS = {
    tp.DVZ_REQUEST_ACTION_CREATE: 'create',
    tp.DVZ_REQUEST_ACTION_DELETE: 'delete',
    tp.DVZ_REQUEST_ACTION_RESIZE: 'resize',
    tp.DVZ_REQUEST_ACTION_UPDATE: 'update',
    tp.DVZ_REQUEST_ACTION_BIND: 'bind',
    tp.DVZ_REQUEST_ACTION_UPLOAD: 'upload',
    tp.DVZ_REQUEST_ACTION_UPFILL: 'upfill',
    tp.DVZ_REQUEST_ACTION_DOWNLOAD: 'download',
    tp.DVZ_REQUEST_ACTION_SET: 'set',
    tp.DVZ_REQUEST_ACTION_GET: 'get',
}

_OBJECTS = {
    tp.DVZ_REQUEST_OBJECT_BOARD: 'board',
    tp.DVZ_REQUEST_OBJECT_CANVAS: 'canvas',
    tp.DVZ_REQUEST_OBJECT_DAT: 'dat',
    tp.DVZ_REQUEST_OBJECT_TEX: 'tex',
    tp.DVZ_REQUEST_OBJECT_SAMPLER: 'sampler',
    tp.DVZ_REQUEST_OBJECT_COMPUTE: 'compute',
    tp.DVZ_REQUEST_OBJECT_GRAPHICS: 'graphics',
    # tp.DVZ_REQUEST_OBJECT_BEGIN: 'begin',
    # tp.DVZ_REQUEST_OBJECT_VIEWPORT: 'viewport',
    # tp.DVZ_REQUEST_OBJECT_VERTEX: 'vertex',
    # tp.DVZ_REQUEST_OBJECT_BARRIER: 'barrier',
    # tp.DVZ_REQUEST_OBJECT_DRAW: 'draw',
    # tp.DVZ_REQUEST_OBJECT_END: 'end',
}
...
cdef class Requester:
    ...
    def create_graphics(self, DvzId board_id, DvzGraphicsType type, int id=0, int flags=0):
        logger.debug(
            f"create graphics, type={type}, board={board_id}, id={id}, flags={flags}")
        cdef rq.DvzRequest req = rq.dvz_create_graphics(self._c_rqr, type, flags)
        if id != 0:
            req.id = id
        rq.dvz_requester_add(self._c_rqr, req)
        return req.id
    ...
    def create_dat(self, DvzBufferType type, DvzSize size, int id=0, int flags=0):
        logger.debug(
            f"create dat, type={type}, size={size}, id={id}, flags={flags}")
    ...
    def bind_tex(self, DvzId pipe, int slot_idx, DvzId tex, DvzId sampler):
        logger.debug(
            f"bind tex, pipe={pipe}, slot_idx={slot_idx}, tex={tex}, sampler={sampler}")
    ...
    def upload_dat(self, DvzId dat, DvzSize offset, np.ndarray data):
        logger.debug(
            f"upload dat, dat={dat}, offset={offset}, data=<array {data.dtype} with {str(data.size)} items>")
    ...
    def record_viewport(self, DvzId board, int x, int y, int w, int h):
        logger.debug(
            f"record viewport, board={board}, x={x}, y={y}, w={w}, h={h}")
    ...
    def record_draw(self, DvzId board, DvzId graphics, int first_vertex, int vertex_count):
        logger.debug(
            f"record draw, board={board}, graphics={graphics}, first_vertex={first_vertex}, vertex_count={vertex_count}")
