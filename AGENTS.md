...
11. Keep machine-access details out of tracked files and commits unless the user explicitly approves the exact details for inclusion. This includes SSH aliases, private hostnames/IP addresses, login names, and connection instructions derived from the user's environment or conversation. Store them only in ignored local notes; use generic platform labels in shared plans. Review the staged diff for these details before committing. Permission to use a machine is not permission to record its access details in the repository.
12. Treat every new portable public example as one cross-language deliverable: add the canonical C scenario, a runnable Python direct-engine adaptation, and a live WebGPU route with focused packet and browser validation in the same change. Do not leave Python or WebGPU as planned follow-up work unless an intrinsic platform limitation makes that route impossible; record that limitation explicitly in the manifest.
