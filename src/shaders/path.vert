...
layout (std140, binding = 2) uniform PathParams {
    float linewidth;
    float miterLimit;
    int capType;
    int roundJoin;
...
layout (location = 1) out vec2 outCaps;
layout (location = 2) out float outLength;
layout (location = 3) out vec2 outTexcoord;
layout (location = 4) out vec2 outBevelDistance;
...
void main() {
    ...
    // Screen coordinates.
    vec4 p0_ = ortho_inv * transform_pos(p0_ndc);
    vec4 p1_ = ortho_inv * transform_pos(p1_ndc);
    vec4 p2_ = ortho_inv * transform_pos(p2_ndc);
    vec4 p3_ = ortho_inv * transform_pos(p3_ndc);
    ...
    outColor = get_color(color);

    float linewidth = pathParams.linewidth;
    float miterLimit = pathParams.miterLimit;
    ...
    if (index == 0) {
        ...
        if( p2 == p3 ) outCaps.y = outTexcoord.x;
        else           outCaps.y = 1.0;
        gl_Position = ortho * vec4(p, z, 1.0);
        outBevelDistance.x = +d0*line_distance(p1+d0*n0*w, p1+d0*n1*w, p);
        outBevelDistance.y =    -line_distance(p2+d1*n1*w, p2+d1*n2*w, p);
    }
    ...
    if (index == 2) {// || index == 4) {
        ...
        if( p0 == p1 ) outCaps.x = outTexcoord.x;
        else           outCaps.x = 1.0;
        gl_Position = ortho * vec4(p, z, 1.0);
        outBevelDistance.x =    -line_distance(p1+d0*n0*w, p1+d0*n1*w, p);
        outBevelDistance.y = +d1*line_distance(p2+d1*n1*w, p2+d1*n2*w, p);
    }
