 ...
 */
static float _axis_panzoom_scale(const float extent[4], DvzDim dim)
{
    ANN(extent);
    ...
    float scale_y, bool snap)
{
    ...
    if (!(scale_y > 0.0f) || !isfinite(scale_y))
        scale_y = 1.0f;
    float half_x = 0.5f * width_px * _axis_visual_pixel_size(axis, DVZ_DIM_X) / scale_x;
