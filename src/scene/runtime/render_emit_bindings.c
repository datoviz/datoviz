    ...
    DvzFramePlanEmitter* emitter, DvzDrp2CommandStream* stream, uint64_t* out_id)
{
    ...
    if (is_new)
    {
        ...
        if (!dvz_drp2_stream_create_bind_group_layout_entries(stream, id, 4, entries))
            return false;
    }
    *out_id = id;
    return true;
    ...
    DvzColorRole color_role, uint64_t* out_id)
{
    ...
    dvz_snprintf(
        ...
        texture_id, role_key);
    dvz_snprintf(
        bg_key, sizeof(bg_key), "_bg_mesh_textured_%" PRIu64 "_%" PRIu64 "_%" PRIu64 "_r%u",
        material_buffer_id, texture_id, sampler_id, role_key);
    bool is_new = false;
    ...
    if (is_new)
    {
        ...
        if (!dvz_drp2_stream_create_bind_group_entries(
                stream, id, bind_group_layout_id, 4, entries))
        {
            return false;
        }
    }
