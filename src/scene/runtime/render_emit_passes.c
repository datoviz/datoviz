    ...
    const DvzFramePlanEmitConfig* cfg, SceneRenderStateCache* cache, DvzDiagnosticReport* report)
{
    ...
    if (!_render_pass_resolve_scene_color_target(
    ...
    ok = _emitter_prepare_render_multi(
        ...
        color_target_format, sampled_depth_id, false, 0, 0, 0, 0, pass_sample_count,
    ...
    ok = dvz_drp2_stream_begin_command_encoder(stream, encoder_id) &&
         dvz_drp2_stream_begin_render_pass_region_clear(
             ...
             1.0f, clear);
    ok = ok && _stream_apply_graph_color_ops(stream, graph_pass, scene_color_id, NULL);
    ...
    DvzDiagnosticReport* report)
{
    ...
    for (uint32_t i = 0; ok && i < plan->count; i++)
    {
        ...
        ok = _emitter_prepare_render_multi(
            ...
            _render_pass_scene_color_target_format(cfg), 0, false, 0, 0, 0, 0, 1, false, report,
    ...
    DvzDiagnosticReport* report)
{
    ...
    for (uint32_t i = 0; ok && i < plan->count; i++)
    {
        ...
        if (render->u.render.visual_count > 0)
        {
            ...
            ok = _emitter_prepare_render_multi(
                ...
                force_point_depth, _render_pass_color_target_format(plan, render_graph_pass, cfg),
    ...
    for (uint32_t i = 0; ok && i < order_count; i++)
    {
        ...
        else if (provider == DVZ_SCENE_WORK_PROVIDER_WBOIT_ACCUMULATION)
        {
            ...
            DvzPanelDesc render_rect = {0};
            const float transparent_clear[4] = {0};
            ok = _stream_begin_render_pass_graph_rect(
                     stream, plan, graph_pass, render, cfg, pass_id, encoder_id, accum_id,
                     transparent_clear, true, false, &render_rect) &&
        ...
        else if (provider == DVZ_SCENE_WORK_PROVIDER_EDL)
        {
            ...
            ok = _stream_begin_render_pass_graph_rect(
                     stream, plan, graph_pass, render, cfg, pass_id, encoder_id, target_id,
                     edl_clear, true, false, &render_rect) &&
                 _stream_apply_graph_color_ops(
                     stream, graph_pass, scene_color_id, &graph_targets) &&
                 dvz_drp2_stream_set_viewport(
                     stream, pass_id, render_rect.x, render_rect.y, render_rect.width,
        ...
        else if (provider == DVZ_SCENE_WORK_PROVIDER_PRESENTATION)
        {
            ...
            bool clear_panel = graph_pass != NULL && graph_pass->color_attachment_count > 0 &&
                               graph_pass->color_attachments[0].load_op ==
    ...
    const DvzFramePlanNode* readback, const DvzFramePlanEmitConfig* cfg)
{
    ...
    ok = dvz_drp2_stream_begin_command_encoder(stream, encoder_id) &&
         ...
         dvz_drp2_stream_begin_render_pass_clear(
             stream, render_pass_id, encoder_id, scene_color_id, cfg ? cfg->clear_color[0] : 0.0f,
