    ...
    DvzDiagnosticReport* report, const DvzFramePlanEmitConfig* cfg)
{
    ...
    for (uint32_t i = 0; ok && (compute == NULL || scene_compute) && i < plan->count; i++)
    {
        if (plan->nodes[i].type == DVZ_FRAME_PLAN_NODE_UPLOAD)
        {
            ...
            bool texture_upload = plan->nodes[i].u.upload.texture_width > 0 &&
                                  plan->nodes[i].u.upload.texture_height > 0;
            if (ok && !texture_upload &&
                fallback_vertex_buffer_count < DVZ_SCENE_MAX_NODE_RESOURCES)
