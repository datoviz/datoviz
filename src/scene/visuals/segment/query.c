 ...
 */
static bool _segment_query_attr(
...
/**
 * Allocate one temporary segment query buffer with checked size arithmetic.
 *
 ...
 */
static bool _segment_query_alloc(void** out_ptr, uint64_t count, uint64_t item_size)
{
    ANN(out_ptr);
    uint64_t bytes = 0;
    ...
    if (ptr == NULL && bytes > 0)
    {
        log_error("segment query request buffer allocation failed");
        return false;
    }
    *out_ptr = ptr;
    return true;
 ...
 */
static bool _segment_query_target_extent(
 ...
 */
static void _segment_query_apply_render_state(
 ...
 */
static void _segment_query_mark_last_upload_index(DvzFramePlan* plan, uint32_t stride)
{
    ANN(plan);
 ...
 * @param plan the frame plan
 */
static void _segment_query_mark_last_upload_uniform(DvzFramePlan* plan)
{
    ANN(plan);
    DvzFramePlanNode* node = plan->count > 0 ? &plan->nodes[plan->count - 1] : NULL;
    if (node == NULL || node->type != DVZ_FRAME_PLAN_NODE_UPLOAD)
    ...
    uint64_t* out_index_count)
{
    ...
    if (!_segment_query_alloc(
            (void**)&scratch->pick_position_start, vertex_count, 3 * sizeof(float)) ||
        !_segment_query_alloc(
            (void**)&scratch->pick_position_end, vertex_count, 3 * sizeof(float)) ||
        !_segment_query_alloc((void**)&scratch->pick_line_width, vertex_count, sizeof(float)) ||
        !_segment_query_alloc((void**)&scratch->pick_ids, vertex_count, sizeof(uint32_t)) ||
        !_segment_query_alloc((void**)&scratch->pick_indices, index_count, sizeof(uint32_t)))
    {
        return false;
    }
    ...
    for (uint64_t i = 0; i < item_count; i++)
    {
        for (uint32_t j = 0; j < 4; j++)
        {
            ...
            scratch->pick_line_width[dst] = line_width[i];
            scratch->pick_ids[dst] = (uint32_t)i + 1u;
        }
        scratch->pick_indices[6 * i + 0] = (uint32_t)(4 * i + 0);
        scratch->pick_indices[6 * i + 1] = (uint32_t)(4 * i + 1);
        scratch->pick_indices[6 * i + 2] = (uint32_t)(4 * i + 2);
        scratch->pick_indices[6 * i + 3] = (uint32_t)(4 * i + 0);
        scratch->pick_indices[6 * i + 4] = (uint32_t)(4 * i + 2);
        scratch->pick_indices[6 * i + 5] = (uint32_t)(4 * i + 3);
    }
    ...
    const DvzSceneQueryBuildContext* ctx, DvzSceneQueryPlan* out_plan)
{
    ...
    if (!_segment_query_target_extent(ctx->figure, ctx->panel, &target_width, &target_height))
        return false;
    ...
    if (ok)
        _segment_query_mark_last_upload_index(plan, sizeof(uint32_t));
    ok = ok && dvz_frame_plan_upload_bytes(
                   plan, "query0_material", 0, sizeof(DvzSceneMaterialParams), "material_params",
                   &ctx->visual->material_params);
    if (ok)
        _segment_query_mark_last_upload_uniform(plan);
    ...
    if (ok)
        _segment_query_apply_render_state(
