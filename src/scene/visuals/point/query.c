 ...
 */
static bool _point_query_target_extent(
 ...
 */
static void _point_query_apply_render_state(
    ...
    const DvzSceneQueryBuildContext* ctx, DvzSceneQueryPlan* out_plan)
{
    ...
    if (!_point_query_attr(ctx->visual, "position", sizeof(vec3), &pos_attr) ||
        !_point_query_attr(ctx->visual, "color", sizeof(DvzColor), &color_attr) ||
        !_point_query_attr(ctx->visual, "size", sizeof(float), &size_attr))
    {
        return false;
    }
    if (
    ...
    {
        log_error("point query request buffer size overflow");
        return false;
    }

    uint32_t target_width = 0;
    uint32_t target_height = 0;
    if (!_point_query_target_extent(ctx->figure, ctx->panel, &target_width, &target_height))
        return false;

    DvzFramePlan* plan = dvz_frame_plan("figure.query.point", ctx->pending->request.request_id);
    bool ok = plan != NULL;
    ...
    if (ok)
        _point_query_apply_render_state(
    ...
    {
        log_error(
            "point query request %" PRIu64 " failed to assemble the GPU readback plan",
    ...
    const DvzSceneQueryDecodeContext* ctx, DvzQueryResult* out_result)
{
    ...
    out_result->visual_family = DVZ_SCENE_VISUAL_FAMILY_POINT;
...
/**
 * Complete point-family readout fields after decode.
 *
 * @param ctx readout context
 * @param result query result
 * @return true when readout succeeded
 */
static bool _point_query_readout(
