 ...
 */
static bool _primitive_query_attr(
...
/**
 * Allocate one temporary primitive query buffer with checked size arithmetic.
 *
 ...
 */
static bool _primitive_query_alloc(void** out_ptr, uint64_t count, uint64_t item_size)
{
    return _dvz_scene_query_alloc("primitive", out_ptr, count, item_size);
}
 ...
 */
static bool _primitive_query_target_extent(
 ...
 */
static void _primitive_query_apply_render_state(
 ...
 */
static bool _primitive_query_source_vertex_index(
...
/**
 * Build temporary GPU primitive buffers for one retained primitive visual.
 *
    ...
    uint32_t* out_topology)
{
    ...
    if (!_primitive_query_attr(visual, "position", sizeof(vec3), &pos_attr))
        return false;
        ...
        visual->buffer->desc.byte_size > 0 && visual->buffer->desc.stride > 0)
    {
        ...
        {
            log_error("primitive query request index stride must be 16-bit or 32-bit");
            return false;
        }
        if (visual->buffer->desc.byte_size % stride != 0)
        {
            log_error("primitive query request index buffer size is not stride-aligned");
            return false;
        }
...
/**
 * Return whether a primitive visual can answer one query request.
 *
 ...
 */
static bool _primitive_query_eligible(
    const DvzPanel* panel, const DvzVisual* visual, const DvzQueryRequest* request)
{
    ANN(panel);
    return _dvz_scene_query_item_target_eligible(visual, request, DVZ_VISUAL_TYPE_PRIMITIVE);
}



/**
 * Build a primitive-family r32uint item query plan.
 *
 * @param ctx build context
 * @param out_plan output query plan
 * @return true when the plan was assembled
 */
static bool _primitive_query_build(
    const DvzSceneQueryBuildContext* ctx, DvzSceneQueryPlan* out_plan)
{
    ...
    if (ok)
        _primitive_query_apply_render_state(
...
/**
 * Decode a primitive-family r32uint item query payload.
 *
 * @param ctx decode context
 * @param out_result output query result
 * @return true when a terminal result was produced
 */
static bool _primitive_query_decode(
    const DvzSceneQueryDecodeContext* ctx, DvzQueryResult* out_result)
{
    return _dvz_scene_query_decode_item_id(ctx, DVZ_SCENE_VISUAL_FAMILY_PRIMITIVE, out_result);
}



/**
 * Complete primitive-family readout fields after decode.
 *
 * @param ctx readout context
 * @param result query result
 * @return true when readout succeeded
 */
static bool _primitive_query_readout(
...
const DvzSceneQueryFamilyOps* _dvz_scene_query_primitive_ops(void)
{
    static const DvzSceneQueryFamilyOps ops = {
        ...
        .eligible = _primitive_query_eligible,
        .build = _primitive_query_build,
        .decode = _primitive_query_decode,
        .readout = _primitive_query_readout,
    };
    return &ops;
}
