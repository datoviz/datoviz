 ...
 */
static bool _marker_query_attr(
 ...
 */
static bool _marker_query_target_extent(
 ...
 */
static void _marker_query_apply_render_state(
    ...
    const DvzSceneQueryBuildContext* ctx, DvzSceneQueryPlan* out_plan)
{
    ...
    if (!_marker_query_attr(ctx->visual, "position", sizeof(vec3), &pos_attr) ||
        !_marker_query_attr(ctx->visual, "color", sizeof(DvzColor), &color_attr) ||
        !_marker_query_attr(ctx->visual, "size", sizeof(float), &size_attr))
    {
        return false;
    }
    if (
    ...
    {
        log_error("marker query request buffer size overflow");
        return false;
    }

    uint32_t target_width = 0;
    uint32_t target_height = 0;
    if (!_marker_query_target_extent(ctx->figure, ctx->panel, &target_width, &target_height))
        return false;

    DvzFramePlan* plan = dvz_frame_plan("figure.query.marker", ctx->pending->request.request_id);
    bool ok = plan != NULL;
    ...
    if (ok)
        _marker_query_apply_render_state(
    ...
    {
        log_error(
            "marker query request %" PRIu64 " failed to assemble the GPU readback plan",
    ...
    const DvzSceneQueryDecodeContext* ctx, DvzQueryResult* out_result)
{
    ...
    out_result->visual_family = DVZ_SCENE_VISUAL_FAMILY_MARKER;
...
/**
 * Complete marker-family readout fields after decode.
 *
 * @param ctx readout context
 * @param result query result
 * @return true when readout succeeded
 */
static bool _marker_query_readout(
