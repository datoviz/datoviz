...
layout(location = 0) out uvec2 outPayload;
...
uint encode_uvw(vec3 uvw)
{
    uvec3 code = uvec3(floor(clamp(uvw, vec3(0.0), vec3(1.0)) * 1023.0 + 0.5));
    return code.x | (code.y << 10u) | (code.z << 20u);
}
...
void main()
{
    ...
    outPayload = uvec2(encode_scalar(sample_value), encode_uvw(uvw));
}
