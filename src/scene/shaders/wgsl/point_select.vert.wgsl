...
fn main(@builtin(vertex_index) vertex_id: u32, input: VertexIn) -> VertexOut {
    ...
    let selected = select(0.0, 1.0, input.selection != 0u);
    let dim = mix(0.25, 1.0, selected);

    var output: VertexOut;
    output.position = vec4f(center.xy + corner * radius * center.w, center.zw);
    output.color = vec4f(input.color.rgb * dim, mix(input.color.a * 0.25, 1.0, selected));
