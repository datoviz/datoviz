 ...
 */
static bool _vector_required_attrs(const DvzVisual* visual, uint64_t* out_count)
{
    ANN(visual);
    ANN(out_count);
    *out_count = 0;
 ...
 * @return whether the cache is ready for upload
 */
static bool _vector_cache_rebuild(DvzVisual* visual)
{
    ANN(visual);
    uint64_t item_count = 0;
    if (!_vector_required_attrs(visual, &item_count))
        return false;
    uint64_t vertex_count = 0;
    uint64_t index_count = 0;
    if (_dvz_mul_u64_overflows(item_count, 4, &vertex_count) ||
    ...
    for (uint64_t i = 0; i < item_count; i++)
    {
        float start[3] = {0};
        float end[3] = {0};
        _vector_endpoints(visual, &position[3 * i], &vector[3 * i], start, end);
        for (uint32_t j = 0; j < 4; j++)
        {
            uint64_t dst = 4 * i + j;
            dvz_memcpy(
    ...
    const DvzFigure* figure, DvzFramePlan* plan, DvzVisual* visual, uint32_t visual_index)
{
    ...
    if (!dirty)
        return;
    if (!_vector_cache_rebuild(visual))
        return;
