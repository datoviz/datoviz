...
/**
 * Resize a derived cache array still owned by frame-plan lowering.
 *
 ...
 */
static bool _derived_cache_resize(void** ptr, uint64_t count, uint64_t item_size)
{
    ANN(ptr);
    uint64_t bytes = 0;
...
static bool _path_cache_rebuild(DvzVisual* visual)
{
    ...
    if (!_derived_cache_resize((void**)&cache->position_prev, vertex_count, 3 * sizeof(float)) ||
        !_derived_cache_resize((void**)&cache->position_curr, vertex_count, 3 * sizeof(float)) ||
        !_derived_cache_resize((void**)&cache->position_next, vertex_count, 3 * sizeof(float)) ||
        !_derived_cache_resize((void**)&cache->color, vertex_count, sizeof(DvzColor)) ||
        !_derived_cache_resize((void**)&cache->line_width, vertex_count, sizeof(float)) ||
        !_derived_cache_resize((void**)&cache->path_flags, vertex_count, sizeof(uint32_t)) ||
        !_derived_cache_resize((void**)&cache->path_distance, vertex_count, sizeof(float)) ||
        !_derived_cache_resize((void**)&cache->indices, index_count, sizeof(uint32_t)))
    {
...
static bool _image_cache_rebuild(const DvzFigure* figure, DvzVisual* visual)
{
    ...
    if (!_derived_cache_resize((void**)&cache->position, vertex_count, 3 * sizeof(float)) ||
        !_derived_cache_resize((void**)&cache->texcoords, vertex_count, 2 * sizeof(float)))
    {
