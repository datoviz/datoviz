...
/**
 * Ensure an orbit-camera-bound mesh still renders after a hosted pointer drag.
 *
 * @param suite the test suite
 * @param item the test item
 * @return 0 on success
...
int test_app_offscreen_orbit_camera_drag_renders_cube(TstContext* suite, const TstCase* item)
{
    ...
    ANN(orbit);
    AT(dvz_panel_bind_controller(panel, orbit_controller, DVZ_DIM_MASK_XYZ) == 0);
    ...
    {
        log_warn(
            "test_app_offscreen_orbit_camera_drag_renders_cube skipped: GPU context creation "
            "failed");
        tst_skip(suite, "GPU context creation failed");
        dvz_scene_destroy(scene);
        return 0;
    ...
    {
        const uint8_t* pixel = &rgba[4 * i];
        if (pixel[0] > 20 || pixel[1] > 20 || pixel[2] > 30)
            lit_count++;
    }
    AT(lit_count > 64);
...
int test_scene_app(TstSuite* suite)
{
    ...
    TST_SCENE_APP_SHARED_CASE(test_app_offscreen_orbit_camera_drag_renders_cube);
