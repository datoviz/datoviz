...
int triunsuitable(
    triorg, tridest, triapex, area) vertex triorg; /* The triangle's origin vertex. */
...

{
    ...
    pool->nextitem =
        (VOID*)(alignptr + (INT_PTR)pool->alignbytes - (alignptr % (INT_PTR)pool->alignbytes));
...
#ifdef ANSI_DECLARATORS
void poolinit(
    struct memorypool* pool, int bytecount, int itemcount, int firstitemcount, int alignment)
#else  /* not ANSI_DECLARATORS */
...

{
    ...
    poolinit(
        &m->vertices, vertexsize, VERTEXPERBLOCK,
        m->invertices > VERTEXPERBLOCK ? m->invertices : VERTEXPERBLOCK, sizeof(REAL));
}
...

{
    ...
    if (b->usesegments)
    {
        ...
        poolinit(
            &m->subsegs, 8 * sizeof(triangle) + sizeof(int), SUBSEGPERBLOCK, SUBSEGPERBLOCK, 4);
...

{
    ...
    foundvertex =
        (char*)(alignptr + (INT_PTR)m->vertices.alignbytes - (alignptr % (INT_PTR)m->vertices.alignbytes));
...
static
    ...
    REAL
    orient3dadapt(
        vertex pa, vertex pb, vertex pc, vertex pd, REAL aheight, REAL bheight, REAL cheight,
...
static
    ...
    REAL
    orient3d(
        struct mesh* m, struct behavior* b, vertex pa, vertex pb, vertex pc, vertex pd,
...

{
    ...
    {
        return orient3d(
            m, b, pa, pb, pc, pd, pa[0] * pa[0] + pa[1] * pa[1] - pa[2],
...
void findcircumcenter(
    struct mesh* m, struct behavior* b, vertex torg, vertex tdest, vertex tapex,
...

{
    ...
    if (b->verbose > 2)
    {
        ...
        printf(
            ...
            badtri->triangorg[1], badtri->triangdest[0], badtri->triangdest[1],
...
#ifdef ANSI_DECLARATORS
void enqueuebadtri(
    struct mesh* m, struct behavior* b, struct otri* enqtri, REAL minedge, vertex enqapex,
...
enum locateresult preciselocate(
    struct mesh* m, struct behavior* b, vertex searchpoint, struct otri* searchtri,
    ...
    while (1)
    {
        if (b->verbose > 2)
        {
            printf(
                ...
                fdest[0], fdest[1], fapex[0], fapex[1]);
        }
...
#ifdef ANSI_DECLARATORS
enum locateresult
locate(struct mesh* m, struct behavior* b, vertex searchpoint, struct otri* searchtri)
#else  /* not ANSI_DECLARATORS */
...

{
    ...
    while (totalsamplesleft > 0)
    {
        ...
        firsttri =
            (char*)(alignptr + (INT_PTR)m->triangles.alignbytes - (alignptr % (INT_PTR)m->triangles.alignbytes));
...
enum insertvertexresult insertvertex(
    struct mesh* m, struct behavior* b, vertex newvertex, struct otri* searchtri,
    struct osub* splitseg, int segmentflaws, int triflaws)
#else  /* not ANSI_DECLARATORS */
enum insertvertexresult
    insertvertex(m, b, newvertex, searchtri, splitseg, segmentflaws, triflaws) struct mesh* m;
struct behavior* b;
vertex newvertex;
struct otri* searchtri;
        ...
        if (doflip)
        {
            ...
            else
            {
                    ...
                    (leftvertex == m->infvertex3))
                {
                    ...
                    (rightvertex == m->infvertex1) || (rightvertex == m->infvertex2) ||
                    (rightvertex == m->infvertex3))
                {
                    ...
                    (farvertex == m->infvertex1) || (farvertex == m->infvertex2) ||
                ...
                if (doflip)
                {
                    ...
                    if (newvertex != (vertex)NULL)
                    {
                        ...
                        /*
                           ...
                           < 0.0) { printf("Internal error in insertvertex():\n"); printf("
                           Clockwise triangle prior to edge flip (top).\n");
                                    }
                        */
...
void triangulatepolygon(
    struct mesh* m, struct behavior* b, struct otri* firstedge, struct otri* lastedge,
...

{
    ...
    if (b->verbose > 2)
    {
        ...
        printf(
            "    (%.12g, %.12g) (%.12g, %.12g)\n", leftbasevertex[0], leftbasevertex[1],
            rightbasevertex[0], rightbasevertex[1]);
    }
...
#ifdef ANSI_DECLARATORS
void mergehulls(
    struct mesh* m, struct behavior* b, struct otri* farleft, struct otri* innerleft,
...
#ifdef ANSI_DECLARATORS
void divconqrecurse(
    struct mesh* m, struct behavior* b, vertex* sortarray, int vertices, int axis,
...

{
    ...
    else
    {
        ...
        divconqrecurse(
            m, b, &sortarray[divider], vertices - divider, 1 - axis, &innerright, farright);
        if (b->verbose > 1)
        {
            printf(
...
void createeventheap(
    struct mesh* m, struct event*** eventheap, struct event** events, struct event** freeevents)
#else  /* not ANSI_DECLARATORS */
...
void check4deadevent(
    struct otri* checktri, struct event** freeevents, struct event** eventheap, int* heapsize)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
struct splaynode*
splay(struct mesh* m, struct splaynode* splaytree, vertex searchpoint, struct otri* searchtri)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
struct splaynode*
splayinsert(struct mesh* m, struct splaynode* splayroot, struct otri* newkey, vertex searchpoint)
#else  /* not ANSI_DECLARATORS */
...
struct splaynode* circletopinsert(
    struct mesh* m, struct behavior* b, struct splaynode* splayroot, struct otri* newkey,
...

{
    ...
    searchpoint[1] = topy;
    return splayinsert(
        m, splay(m, splayroot, (vertex)searchpoint, &dummytri), newkey, (vertex)searchpoint);
}
...
struct splaynode* frontlocate(
    struct mesh* m, struct splaynode* splayroot, struct otri* bottommost, vertex searchvertex,
    struct otri* searchtri, int* farright)
#else  /* not ANSI_DECLARATORS */
struct splaynode*
    frontlocate(m, splayroot, bottommost, searchvertex, searchtri, farright) struct mesh* m;
struct splaynode* splayroot;
    ...
    while (heapsize > 0)
    {
        ...
        if (nextevent->xkey < m->xmin)
        {
            ...
            if (randomnation(SAMPLERATE) == 0)
            {
                ...
                splayroot = circletopinsert(
                    m, b, splayroot, &lefttri, leftvertex, midvertex, rightvertex,
                    nextevent->ykey);
            }
        }
        else
        {
            ...
            else
            {
                ...
                splayroot =
                    frontlocate(m, splayroot, &bottommost, nextvertex, &searchtri, &farrightflag);
                /*
                        otricopy(bottommost, searchtri);
                        farrightflag = 0;
...
#ifdef ANSI_DECLARATORS
int reconstruct(
    struct mesh* m, struct behavior* b, int* trianglelist, REAL* triangleattriblist,
    REAL* trianglearealist, int elements, int corners, int attribs, int* segmentlist,
    int* segmentmarkerlist, int numberofsegments)
#else  /* not ANSI_DECLARATORS */
int reconstruct(
    m, b, trianglelist, triangleattriblist, trianglearealist, elements, corners, attribs,
    segmentlist, segmentmarkerlist, numberofsegments) struct mesh* m;
struct behavior* b;
int* trianglelist;
...
#ifdef ANSI_DECLARATORS
long reconstruct(
    struct mesh* m, struct behavior* b, char* elefilename, char* areafilename, char* polyfilename,
    FILE* polyfile)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
enum finddirectionresult
finddirection(struct mesh* m, struct behavior* b, struct otri* searchtri, vertex searchpoint)
#else  /* not ANSI_DECLARATORS */
...
void segmentintersection(
    struct mesh* m, struct behavior* b, struct otri* splittri, struct osub* splitsubseg,
    vertex endpoint2)
#else  /* not ANSI_DECLARATORS */
...

{
    ...
    if (b->verbose > 1)
    {
        printf(
            ...
            torg[1], tdest[0], tdest[1], newvertex[0], newvertex[1]);
    }
...
#ifdef ANSI_DECLARATORS
int scoutsegment(
    struct mesh* m, struct behavior* b, struct otri* searchtri, vertex endpoint2, int newmark)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void conformingedge(
    struct mesh* m, struct behavior* b, vertex endpoint1, vertex endpoint2, int newmark)
#else  /* not ANSI_DECLARATORS */
...
void constrainededge(
    struct mesh* m, struct behavior* b, struct otri* starttri, vertex endpoint2, int newmark)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void formskeleton(
    struct mesh* m, struct behavior* b, int* segmentlist, int* segmentmarkerlist,
    int numberofsegments)
#else  /* not ANSI_DECLARATORS */
    ...
    while (virusloop != (triangle**)NULL)
    {
        ...
        if (b->verbose > 2)
        {
            ...
            printf(
                ...
                deadorg[1], deaddest[0], deaddest[1], deadapex[0], deadapex[1]);
        }
...

{
    ...
    while (virusloop != (triangle**)NULL)
    {
        ...
        if (b->verbose > 2)
        {
            ...
            printf(
                "    Checking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", regionorg[0],
                regionorg[1], regiondest[0], regiondest[1], regionapex[0], regionapex[1]);
        }
        /* Check each of the triangle's three neighbors. */
        for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
        {
...
#ifdef ANSI_DECLARATORS
void carveholes(
    struct mesh* m, struct behavior* b, REAL* holelist, int holes, REAL* regionlist, int regions)
#else  /* not ANSI_DECLARATORS */
...

{
        ...
        (bapex == badtri->triangapex))
    {
        if (b->verbose > 1)
        {
            ...
            printf(
                "    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", borg[0], borg[1], bdest[0],
                bdest[1], bapex[0], bapex[1]);
        }
...

{
    ...
    if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest)
    {
        ...
        poolinit(
            &m->flipstackers, sizeof(struct flipstacker), FLIPSTACKERPERBLOCK, FLIPSTACKERPERBLOCK,
            0);
        m->checkquality = 1;
        if (b->verbose)
        {
...
#ifdef ANSI_DECLARATORS
void readnodes(
    struct mesh* m, struct behavior* b, char* nodefilename, char* polyfilename, FILE** polyfile)
#else  /* not ANSI_DECLARATORS */
...
#else  /* not ANSI_DECLARATORS */
    void transfernodes(
        m, b, pointlist, pointattriblist, pointmarkerlist, numberofpoints,
...
#ifdef ANSI_DECLARATORS
void readholes(
    struct mesh* m, struct behavior* b, FILE* polyfile, char* polyfilename, REAL** hlist,
    int* holes, REAL** rlist, int* regions)
#else  /* not ANSI_DECLARATORS */
...

{
    ...
    fprintf(
        outfile, "%ld  %d  %d\n", m->triangles.items, (b->order + 1) * (b->order + 2) / 2,
        m->eextras);
#endif /* not TRILIBRARY */
    ...
    while (triangleloop.tri != (triangle*)NULL)
    {
        ...
        if (b->order == 1)
        {
            ...
            fprintf(
                outfile, "%4ld    %4d  %4d  %4d", elementnumber, vertexmark(p1), vertexmark(p2),
                vertexmark(p3));
#endif /* not TRILIBRARY */
        }
        else
        {
            ...
            fprintf(
                outfile, "%4ld    %4d  %4d  %4d  %4d  %4d  %4d", elementnumber, vertexmark(p1),
                vertexmark(p2), vertexmark(p3), vertexmark(mid1), vertexmark(mid2),
                vertexmark(mid3));
#endif /* not TRILIBRARY */
        }
...
#ifdef ANSI_DECLARATORS
void writepoly(
    struct mesh* m, struct behavior* b, char* polyfilename, REAL* holelist, int holes,
    REAL* regionlist, int regions, int argc, char** argv)
#else  /* not ANSI_DECLARATORS */
void
    writepoly(m, b, polyfilename, holelist, holes, regionlist, regions, argc, argv) struct mesh* m;
struct behavior* b;
char* polyfilename;
REAL* holelist;
int holes;
    ...
    if (regions > 0)
    {
        ...
        for (regionnumber = 0; regionnumber < regions; regionnumber++)
        {
            ...
            fprintf(
                ...
                regionlist[4 * regionnumber + 2], regionlist[4 * regionnumber + 3]);
        }
    }
#endif /* not CDT_ONLY */
...
#ifdef ANSI_DECLARATORS
void writevoronoi(
    struct mesh* m, struct behavior* b, REAL** vpointlist, REAL** vpointattriblist,
    int** vpointmarkerlist, int** vedgelist, int** vedgemarkerlist, REAL** vnormlist)
#else  /* not ANSI_DECLARATORS */
    void writevoronoi(
        m, b, vpointlist, vpointattriblist, vpointmarkerlist, vedgelist, vedgemarkerlist,
...
#ifdef ANSI_DECLARATORS
void writevoronoi(
    struct mesh* m, struct behavior* b, char* vnodefilename, char* vedgefilename, int argc,
    char** argv)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void writeneighbors(
    struct mesh* m, struct behavior* b, char* neighborfilename, int argc, char** argv)
#else  /* not ANSI_DECLARATORS */
...

{
    ...
    printf(
        ...
        aspecttable[0], ratiotable[7], ratiotable[8], aspecttable[8]);
    for (i = 1; i < 7; i++)
    {
        printf(
            "  %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n", ratiotable[i - 1],
            ratiotable[i], aspecttable[i], ratiotable[i + 7], ratiotable[i + 8],
            aspecttable[i + 8]);
    }
    printf(
        "  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n", ratiotable[6],
        ratiotable[7], aspecttable[7], ratiotable[14], aspecttable[15]);
    ...
    for (i = 0; i < 9; i++)
    {
        printf(
            ...
            angletable[i], i * 10 + 90, i * 10 + 100, angletable[i + 9]);
    }
    printf("\n");
}
...
#ifdef ANSI_DECLARATORS
void triangulate(
    char* triswitches, struct triangulateio* in, struct triangulateio* out,
...

{
    ...
    if (b.refine)
    {
        ...
        m.hullsize = reconstruct(
            &m, &b, in->trianglelist, in->triangleattributelist, in->trianglearealist,
...
#else  /* not TRILIBRARY */
        m.hullsize =
            reconstruct(&m, &b, b.inelefilename, b.areafilename, b.inpolyfilename, polyfile);
#endif /* not TRILIBRARY */
    }
    else
    {
...
#else  /* not TRILIBRARY */
        readholes(
            &m, &b, polyfile, b.inpolyfilename, &holearray, &m.holes, &regionarray, &m.regions);
#endif /* not TRILIBRARY */
        if (!b.refine)
        {
...
#else  /* not TRILIBRARY */
            writepoly(
                &m, &b, b.outpolyfilename, holearray, m.holes, regionarray, m.regions, argc, argv);
#endif /* not TRILIBRARY */
        }
    }
#ifndef TRILIBRARY
...
#ifdef TRILIBRARY
        writevoronoi(
            &m, &b, &vorout->pointlist, &vorout->pointattributelist, &vorout->pointmarkerlist,
