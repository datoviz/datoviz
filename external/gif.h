...
// Very minor modifications made by Cyrille Rossant for C compatibility (just a convenience)
...
static void GifGetClosestPaletteColor(
...
static int
GifPartition(uint8_t* image, const int left, const int right, const int elt, int pivotIndex)
{
...
static void GifSplitPalette(
    uint8_t* image, int numPixels, int firstElt, int lastElt, int splitElt, int splitDist,
    int treeNode, bool buildForDither, GifPalette* pal)
{
    ...
    GifSplitPalette(
        image, subPixelsA, firstElt, splitElt, splitElt - splitDist, splitDist / 2, treeNode * 2,
        buildForDither, pal);
    GifSplitPalette(
        image + subPixelsA * 4, subPixelsB, splitElt, lastElt, splitElt + splitDist, splitDist / 2,
...
static void GifMakePalette(
    const uint8_t* lastFrame, const uint8_t* nextFrame, uint32_t width, uint32_t height,
...
static void GifDitherImage(
    const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width,
...
static void GifWriteLzwImage(
    FILE* f, uint8_t* image, uint32_t left, uint32_t top, uint32_t width, uint32_t height,
...
// int32_t bitDepth = 8, bool dither = false)
{
    ...
    writer->delay = delay;
    // int32_t bitDepth = 8;
    // bool dither = false; // Mute "Unused argument" warnings
...
static bool GifWriteFrame(
    GifWriter* writer, const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay)
