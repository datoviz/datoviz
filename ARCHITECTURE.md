...
- Near-term development should harden the current point-based scene path — now including retained
  ...
  runtime viewport/scissor handling — `dvz_primitive` for topology-driven rendering has landed;
  the next minimal visual family is image/texture. Keep examples current, and preserve the
  runtime boundary:
