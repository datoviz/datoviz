...
int test_segment_1(TstSuite* suite)
{
    ...
    DvzVisual* segment = dvz_segment(rqr, 0);
    ...
    dvz_segment_position(segment, 0, n, pos, 0);
    ...
    dvz_segment_color(segment, 0, n, color, 0);

    // Important: upload the data to the GPU.
    dvz_visual_update(segment);
    ...
    dvz_visual_mvp(segment, &mvp);

    // Viewport.
    DvzViewport viewport = dvz_viewport_default(WIDTH, HEIGHT);
    dvz_visual_viewport(segment, &viewport);
    ...
    dvz_visual_instance(segment, board_id, 0, 0, n, 0, 1);
