...
//     VK_BUFFER_USAGE_TRANSFER_DST_BIT); dvz_buffer_vma_usage(&buffer, VMA_MEMORY_USAGE_CPU_ONLY);
//     // dvz_buffer_memory(
...
//     dvz_cmd_begin(&cmds, 0);
//     float power = 2.0f;
//     dvz_cmd_push(&cmds, 0, &compute.slots, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(float),
//     &power); dvz_cmd_compute(&cmds, 0, &compute, (uvec3){20, 1, 1}); dvz_cmd_end(&cmds, 0);
//     dvz_cmd_submit_sync(&cmds, 0);
...
//     dvz_sampler_address_mode(&sampler, DVZ_SAMPLER_AXIS_U,
