...
2. Start from the [Examples](examples/index.md), especially examples whose pages include complete
   source code.
3. Check the [C API reference](reference/c-api/index.md), [Python with NumPy arrays](reference/python-direct-engine.md),
   or [Python raw ctypes](reference/ctypes.md) before writing code.
...
Until that layer is ready, Python users can use the Datoviz `ctypes` API directly: the documented
`import datoviz as dvz` path for supported NumPy array uploads, or `datoviz.raw` when exact generated
...
- [Choose your layer](start/choose-your-layer.md): Python, C, raw `ctypes`, WebGPU, and GSP/VisPy2 boundaries.
- [C API reference](reference/c-api/index.md): exact native API names and signatures.
- [Python with NumPy arrays](reference/python-direct-engine.md): supported Python convenience calls.
- [Python raw ctypes](reference/ctypes.md): exact generated low-level Python binding.
