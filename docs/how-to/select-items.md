...
Linked selection compares the scene-local `link_channel` id together with `link_key`; key zero is valid and equal keys on different channels remain independent. `dvz_link_channel_id()` returns zero for NULL or destroyed channels.
