...
- The image in the already-published PyPI project description is broken because that immutable RC2 metadata used a repository-relative URL. The package files and documentation website are unaffected. PyPI does not permit replacing this release metadata or reusing the uploaded filenames, so future releases use an absolute image URL instead of issuing a cosmetic replacement version.
