...
| Qt/PyQt hosted rendering | supported from source; no packaged provider in RC1 | [Platform support](platform-support.md) | Native Qt and PyQt hosting render into Qt-owned Vulkan surfaces without linking Qt into `libdatoviz`. RC1 has no packaged bridge provider; the wheels' `datoviz[qt]` extra installs PyQt6 but not `datoviz_qtbridge`. Packaging is planned for RC2. |
