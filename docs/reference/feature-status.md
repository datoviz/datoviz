...
| Qt/PyQt hosted rendering | supported from source; packaged provider deferred to RC4 | [Platform support](platform-support.md) | Native Qt and PyQt hosting render into Qt-owned Vulkan surfaces without linking Qt into `libdatoviz`. The wheels' `datoviz[qt]` extra installs PyQt6 but not `datoviz_qtbridge`; the official conda provider awaits compatible Vulkan-enabled PyQt publication. |
