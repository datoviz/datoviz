...
| Python with NumPy arrays | supported | [Python with NumPy arrays](python-direct-engine.md) | `import datoviz as dvz` uses the same `dvz_*` function names as the C examples and accepts NumPy arrays for supported visual-data uploads and RGBA capture helpers. |
