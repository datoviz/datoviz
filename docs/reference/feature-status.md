...
| CUDA external-memory buffers | experimental; Linux/NVIDIA only | [GPU array interoperability](gpu-array-interop.md) | Native C and CuPy routes share Datoviz-owned Vulkan buffers through opaque FDs and timeline semaphores. PyTorch and Taichi adapters are implemented but await physical GPU proof. |
