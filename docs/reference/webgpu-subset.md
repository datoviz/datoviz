...
- `examples/webgpu/examples.html?demo=wasm-2d`: point + pixel + marker + primitive + image + mesh +
  panzoom;
...
| Scene visual parity | limited to point, pixel, basic marker, primitive, RGBA8 image, and basic mesh demos |
...
- fixture dashboard WASM Scene Smoke section: 2D point/pixel/marker update, 2D image texture
  resize reload, and 3D mesh update rows pass;
- 2D WASM example: point, pixel, marker, primitive, image, and mesh content render; pan/zoom and
  resize work;
