...
static VkyComputePipeline cpipeline;
...
int main()
{
    ...
    // 1. Graphics pipeline.
    {
        ...
        // Typically one uses one giant vertex buffer for the whole application and allocates parts
        // of the buffer to different visuals.
        VkyBuffer buffer = vky_create_buffer(
            canvas->gpu, size,
            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, 0);
    ...
    // 2. Compute pipeline.
    {
        ...
        // Create the pipeline.
        cpipeline =
            vky_create_compute_pipeline(canvas->gpu, "triangle_rotate.comp.spv", resource_layout);

        // Bind the resources.
        VkyBufferRegion* resources[] = {&vertex_buffer};
        vky_bind_resources(
            cpipeline.resource_layout, cpipeline.descriptor_sets, (void**)resources);

        vky_begin_compute(canvas->gpu);
        vky_compute_acquire(
            &cpipeline, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &vertex_buffer,
            VK_PIPELINE_STAGE_VERTEX_SHADER_BIT);
        vky_compute(&cpipeline, 3, 1, 1);
        vky_compute_release(
            &cpipeline, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &vertex_buffer,
            VK_PIPELINE_STAGE_VERTEX_SHADER_BIT);
        VkDescriptorType descriptor_types[] = {VK_DESCRIPTOR_TYPE_STORAGE_BUFFER};
    ...
    vky_destroy_buffer(vertex_buffer.buffer);
