...
/* visual_segment_stress_glfw - live GLFW stress controls for segment visuals.
 *
 * Build:  just example-c visual_segment_stress_glfw
 * Run:    ./build/examples/c/visual_segment_stress_glfw
 * Smoke:  ./build/examples/c/visual_segment_stress_glfw 120
 */
...
int main(int argc, char** argv)
{
    ...
    state.panel = dvz_panel(figure, (DvzPanelDesc){0.0f, 0.0f, 1.0f, 1.0f});
    if (state.panel == NULL)
    {
    ...
    state.win = dvz_app_window_glfw(app, figure, WIDTH, HEIGHT, "visual_segment_stress_glfw");
    if (state.win == NULL)
    {
