 ...
 * real Earth and Mars textures from the data submodule when available, with generated fallbacks so
 * the example still runs without optional data.
 *
...
#define MARS_TEXTURE_FALLBACK_PATH  "data/textures/mars.jpg"
...
/**
 * Fill one procedural Mars-like equirectangular RGBA texture.
 *
 * @param pixels output RGBA8 texture buffer
 * @param width texture width
 * @param height texture height
 */
static void _make_mars_texture(uint8_t* pixels, uint32_t width, uint32_t height)
{
    ANN(pixels);
    ASSERT(width > 1);
    ASSERT(height > 1);
    ...
    for (uint32_t y = 0; y < height; y++)
    {
        ...
        for (uint32_t x = 0; x < width; x++)
        {
            ...
            const double lon = (u - 0.5) * 2.0 * M_PI;
            const double plains =
                0.58 + 0.22 * sin(5.0 * lon + 1.4 * sin(3.0 * lat)) +
                0.12 * sin(13.0 * lon - 7.0 * lat) + 0.08 * sin(29.0 * (lon + lat));
            const double highlands = _clamp01(0.5 + 0.5 * sin(2.2 * lon - 3.7 * lat));
            const double dust = _clamp01(0.5 + 0.5 * sin(17.0 * lon + 11.0 * lat));
            const bool polar = fabs(lat) > 1.34;
            const double canyon =
                exp(-32.0 * (lat + 0.18) * (lat + 0.18)) *
                exp(-2.2 * (lon + 1.25) * (lon + 1.25));

            double r = 0.47 + 0.22 * plains + 0.08 * highlands;
            double g = 0.21 + 0.12 * plains + 0.06 * dust;
            double b = 0.12 + 0.07 * highlands;
            ...
            if (polar)
            {
                const double cap = _clamp01((fabs(lat) - 1.34) / 0.22);
                r = r * (1.0 - cap) + 0.86 * cap;
                g = g * (1.0 - cap) + 0.76 * cap;
                b = b * (1.0 - cap) + 0.64 * cap;
            }
...
/**
 * Fill one procedural planet texture.
 *
 ...
 * @param height texture height
 */
static void _make_planet_texture(
    PlanetKind kind, uint8_t* pixels, uint32_t width, uint32_t height)
{
    switch (kind)
    {
    case PLANET_MARS:
        _make_mars_texture(pixels, width, height);
        break;
    case PLANET_EARTH:
    default:
        _make_earth_texture(pixels, width, height);
        break;
    }
}
    ...
    DvzScene* scene, PlanetKind kind, PlanetTexture* texture)
{
    ...
    if (texture->pixels == NULL)
    {
        ...
        if (texture->pixels == NULL)
            return false;
        _make_planet_texture(kind, texture->pixels, texture->width, texture->height);
    }
 ...
 * @param state example state
 */
static void _state_apply_planet(TexturedPlanetState* state)
{
    ANN(state);
    ...
    if (texture->field != NULL)
        (void)dvz_visual_set_field(state->visual, "texture", texture->field);
}
