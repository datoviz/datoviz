...
/* replay_dvzr_glfw — replay a DVZR recording into a live GLFW window.
 *
 * Build:  just build
 * Run:    ./build/examples/c/replay_dvzr_glfw path/to/recording.dvzr
 * Loop:   ./build/examples/c/replay_dvzr_glfw --loop path/to/recording.dvzr
 * Fast:   ./build/examples/c/replay_dvzr_glfw --fast path/to/recording.dvzr
 */
...
static void _usage(const char* argv0)
{
    dvz_fprintf(
        ...
        argv0 != NULL ? argv0 : "replay_dvzr_glfw");
}
