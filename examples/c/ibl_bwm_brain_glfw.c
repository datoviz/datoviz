...
/* ibl_bwm_brain_glfw - BWM cluster cloud inside a transparent Allen brain shell.
 *
 * Prepare: python examples/c/prepare_ibl_bwm.py
 * Build:   just example-c ibl_bwm_brain_glfw
 * Run:     ./build/examples/c/ibl_bwm_brain_glfw
 */
...
static void _print_prepare_hint(const char* data_dir)
{
    dvz_fprintf(
        ...
        "prepare it with: python examples/c/prepare_ibl_bwm.py\n",
...
static void _apply_point_size(BwmExampleState* state)
{
    ...
    if (dvz_visual_set_data(
            state->point, "size", state->dataset->cluster_size,
...
int main(void)
{
    ...
    if (dvz_visual_set_data(point, "position", dataset.cluster_pos, dataset.cluster_count) != 0 ||
        ...
        dvz_visual_set_data(point, "size", dataset.cluster_size, dataset.cluster_count) != 0 ||
    ...
    DvzAppWindow* win = dvz_app_window_glfw(app, figure, WIDTH, HEIGHT, "ibl_bwm_brain_glfw");
    if (win == NULL)
    {
