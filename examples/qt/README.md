...
This directory contains optional Qt examples that exercise the external host-window contract without
adding Qt to `libdatoviz`.
...
The Qt surface is borrowed by Datoviz. The adapter passes `owned_by_datoviz = false` in
`DvzWindowExternalSurfaceInfo`, so Datoviz must not destroy the `VkSurfaceKHR`.
...
The cleanup pass may log that the canvas surface is unavailable. That warning is expected in these
examples; Vulkan validation errors during that path are not expected.
