...
// Clear color that can be changed by the Dear ImGui color picker.
static vec4 color;

static void fill_command_buffer(VkyCanvas* canvas, VkCommandBuffer cmd_buf)
{
    // Begin the render pass.
    vky_begin_render_pass(
        cmd_buf, canvas, (VkClearColorValue){{color[0], color[1], color[2], color[3]}});
...
{
    // HACK: for performance reasons we only refill the command buffer if the user is currently
    // modifying the color picker, which is done with mouse dragging.
...
int main()
{
    ...
    // GUI: clear color picker.
    VkyGui* gui = vky_create_gui(canvas, (VkyGuiParams){0, 0});
    vky_gui_control(gui, VKY_GUI_COLOR, "clear color", NULL, (float*)color);
    ...
    // At every frame, mark the command buffer as in need to be recreated so that the clear color
    // can be refreshed.
    ...
    // Only required when using he app interface, not the scene interface.
    vky_destroy_guis();
