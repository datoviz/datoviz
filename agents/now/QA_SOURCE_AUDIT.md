...
Status: active incremental source audit completed through `input`; `math` is next. Updated: 2026-08-02.
...
1. Audit the active `math` module next. Map production consumers and focused tests across `anim`, `box`, `parallel`, `prng`, `stats`, and `vec` before editing.
2. Inspect checked arithmetic, count and stride handling, empty inputs, non-finite values, divide-by-zero and normalization boundaries, interpolation endpoints, PRNG state/range contracts, and allocation ownership.
3. Run narrow normal math tests, focused `clang-tidy` and `cppcheck`, then ASan/UBSan/LSan. Add TSan only for a meaningful Datoviz-owned concurrency path in `parallel`.
4. Commit only actionable fixes and focused regression tests. If a component is clean, record its evidence without manufacturing a code change.
5. After `math`, revisit the incompletely audited `window` boundary before moving through `canvas`, `stream`, `video`, `vk`, `vklite`, `drp2`, `scene`, and `app`. Reorder only when a finding or release blocker establishes a better dependency path.
