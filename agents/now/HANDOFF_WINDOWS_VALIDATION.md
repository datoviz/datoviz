...
Candidate `2c32d3e92fac41785ea063e6677bd4ae688f57cf` results are complete on the 1920×1080 144 Hz production-default AMD path. One slot measured -1.94% paired p95 input-to-submit, two slots -0.25%, and auto with three slots +0.16%; all verdicts were no material change and all confidence intervals crossed or reached zero. Required Canvas cases passed on AMD and NVIDIA with no validation messages. Scripted real-input scatter smokes rendered, resized, returned to idle, and closed normally for all configurations, and the maintainer reported that all three felt very smooth. The evidence recommends one slot as the cross-platform default-policy candidate because it passes and does not regress Windows while matching the Linux direction; changing the default still requires the separate policy decision. The retained external summary and unmodified reports are under `Documents/Codex/2026-08-01/je-configure-ce-pc-windows-11/outputs/frame-slot-comparison-20260802-104405/`.
