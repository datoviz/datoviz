...
The polygon-descriptor caveat is resolved for new examples: `DvzPolygonRing` and `DvzPolygonDesc`
now have generated ctypes layouts, and `dvz_polygon_desc()` is emitted as a generated by-value
helper. Commit `6fc17a0cc` also fixed pointer-to-vector ctypes generation so `dvec2*` and similar
borrowed vector arrays map to one `ctypes.c_void_p`, not an array of pointers. It passed
`just ctypes`, `just ctypes-check`, `just ctypes-smoke`, and `just docs-api-check`.
...
- `composites_polygon` is committed: it has `examples/python/gallery/composites/polygon.py` and a
...
- `showcases_gpu_particle_smoke` is committed: it has
  `examples/python/gallery/showcases/gpu_particle_smoke.py` and a matching `python.source` manifest
  entry. It keeps the real scene-buffer plus scene-compute workflow with position/velocity/age
  storage buffers, point visual attribute buffers, blended rendering, and a frame callback that
...
- `showcases_brain_volume` is committed: it has
  `examples/python/gallery/showcases/brain_volume.py` and a matching `python.source` manifest
  entry. It reads the prepared Allen mouse brain RGBA `.npy.gz` from the `data` submodule,
  downsamples by strongest voxel with the C example's factor-2 policy, derives an R8 scalar tissue
  volume, and renders the same field through composite and slice volume visuals with axis mapping,
