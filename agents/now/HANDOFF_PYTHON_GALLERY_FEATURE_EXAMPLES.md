...
next planned checkpoint is `features_controller_fly`.
...
- v0.4-required feature examples: 44 of 64 have Python entries; 20 remain missing.
- all v0.4-required public examples: 56 of 95 have Python entries; 39 remain missing.
...
`features_controller_fly`, which is the first missing feature in manifest order. Keep the
...
```text
examples: add Python fly controller gallery example
```
