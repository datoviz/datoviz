...
- Strict system/Homebrew source-install proof now passes after installing Homebrew `glfw 3.4`,
  ...
  commits `2c8a49f3d` and `7faf7f7dd`.
- `just distribution-validate-local audit` passes against the strict source install prefix. The
  audit sees 119 installed headers, `libdatoviz*.dylib`, `DatovizConfig.cmake`, `datoviz.pc`,
  Vulkan SDK include flags in pkg-config metadata, and working installed CMake/pkg-config
  consumers. vcpkg and conda audits are skipped because their local prefixes are absent.
