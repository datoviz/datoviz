...
0. Before the cutover, complete the narrow core-independence prerequisite in [`../../spec/data/ASSET_ARCHITECTURE.md`](../../spec/data/ASSET_ARCHITECTURE.md): admit the complete reviewed Source family and Noto scientific fallback directly while preserving the five primary roles and custom-font behavior, replace external generic test inputs, split hermetic documentation checks from data-backed site publication, and prove build, core tests, documentation checks, and release artifacts with the `data` submodule uninitialized. Do not copy the obsolete legacy font set into active parent Git as an intermediate step. Keep `datoviz/data` unchanged during the branch rename; downloader implementation, catalog publication, broad example/gallery migration, and submodule retirement remain separately reviewable post-cutover work rather than new RC3 cutover blockers.
