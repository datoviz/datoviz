...
0. Before the cutover, complete the narrow core-independence prerequisite in [`../../spec/data/ASSET_ARCHITECTURE.md`](../../spec/data/ASSET_ARCHITECTURE.md). The reviewed Source/Noto family, deterministic Source atlas products, five primary roles, custom-font behavior, source-defined generic file fixtures, no-data build, isolated source-package install, and authored core/wheel CI removal of `data` and LFS hydration are complete. Remaining prerequisite proof runs the hosted core workflows and final wheel/release artifacts from the exact candidate with `data` uninitialized and confirms the hermetic documentation versus data-backed site-publication boundary. Keep `datoviz/data` unchanged during the branch rename; downloader implementation, catalog publication, broad example/gallery migration, and submodule retirement remain separately reviewable post-cutover work rather than new RC3 cutover blockers.
