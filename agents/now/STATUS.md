...
The optional Qt bridge and local Apple Silicon split-package proof are complete. Vulkan-enabled Qt 6.11.1 build 2 is published for both macOS architectures. The compatible PyQt PR is ready for review after its final 25-job Linux, macOS, and Windows matrix passed against the published Qt packages; PyQt merge and publication followed by exact Datoviz provider artifacts still block the required RC3 provider lane.
