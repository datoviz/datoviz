...
The optional Qt bridge and local Apple Silicon split-package proof are complete. Vulkan-enabled Qt 6.11.1 build 2 is published for both macOS architectures, and the compatible PyQt PR's fresh fifteen-job matrix is running against it. PyQt merge and publication followed by exact Datoviz provider artifacts still block the required RC3 provider lane.
...
| Qt/PyQt provider | Local Qt 6.11.1 build 2, PyQt6 6.11.0 build 3, split Datoviz packages, Vulkan instance, Cocoa surface, and hosted rendering proof are green; upstream Qt build 2 is published for both macOS architectures, and fresh PyQt Azure build `1569455` is running. | Require the complete PyQt matrix to pass, publish compatible PyQt, then build and validate exact split Datoviz artifacts on supported hosted platforms. |
