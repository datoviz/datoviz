...
GitHub issue #137 exposes a genuine zoom-dependent SSAO correctness defect rather than expected MSAA behavior. A review-draft render-product and technique-composition architecture plus a two-lane rendering/QA execution map now await maintainer decisions; no implementation is authorized yet.
...
| Release quality | Native, hosted, WebGPU, query, compute, documentation, gallery, and GPU-selection evidence exists. Validation-off sanitizer configuration is durable, the incremental source audit is complete through `input`, the remaining QA campaign is mapped, and issue #137 has a review-draft deep rendering architecture and two-lane execution plan. | Review the render-product decisions and QA/render orchestration, run the safe pre-landing QA slices alongside the approved render refactor, then run affected/deferred QA from the render landing manifest before the exact RC3 static-analysis, sanitizer where practical, Vulkan, long-loop, docs, gallery, example, source-archive, wheel, and installed-consumer gates; record limitations explicitly. |
