...
| WebGPU/WASM experimental path | WebGPU fixture runner works; the generic WASM scene ABI and split DRP2 packet path now register 89 scenarios and expose 84 browser-live gallery routes. Streaming DAQ has native and packet proof; its filtered browser run hit the known external headless instance-loss skip. Cortical Activity, SVG Tiger, and Terrain Relief compile and emit cache-backed portable streams, while public routes remain blocked on approved committed browser bundles or artwork clearance. | Obtain manual website confirmation for DAQ/galaxy/scatter/datetime/marker symbols, package the approved Terrain Relief and Cortical Activity browser bundles, resolve SVG artwork redistribution, then continue the ordered coverage program with generic volume, techniques, and splat/point-cloud batches. |
