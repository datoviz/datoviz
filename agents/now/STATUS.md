...
| Release quality | Native, hosted, WebGPU, query, compute, documentation, gallery, and GPU-selection evidence exists. Validation-off sanitizer configuration is durable, and the incremental source audit is complete through `input`; see [QA_SOURCE_AUDIT.md](QA_SOURCE_AUDIT.md). | Continue with `math`, then run the exact RC3 static-analysis, sanitizer where practical, Vulkan, long-loop, docs, gallery, example, source-archive, wheel, and installed-consumer gates; record limitations explicitly. |
