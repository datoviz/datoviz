...
| WebGPU/WASM experimental path | WebGPU fixture runner works; the generic WASM scene ABI and split DRP2 packet path now register 88 scenarios and expose 84 browser-live gallery routes. Streaming DAQ has native and packet proof; its filtered browser run hit the known external headless instance-loss skip. SVG Tiger and Terrain Relief compile and emit cache-backed portable streams, while public routes remain blocked on approved committed browser bundles. | Obtain manual website confirmation for DAQ/galaxy/scatter/datetime/marker symbols, package the approved Terrain Relief browser bundle, resolve SVG artwork redistribution, then continue the ordered coverage program with generic volume, techniques, and splat/point-cloud batches. |
