...
| Gallery media pipeline | Commits `2f0a9365a` and `c6ed19a50` implement canonical `1280x720` media, the bounded FPS/CRF ladder, encoded-media freshness validation, isolated workspaces, deterministic bounded parallelism, and separate capture-worker limits. Published media still reflects the pre-change build-local generation state. | Refresh build-local animation and card media under the new policy, run the full freshness gate, inspect candidates, and obtain exact approval before any canonical media or `data` update. |
...
| Documentation and gallery freeze | Public RC2 documentation is deployed, the three-chapter Vulkan pilot includes approved canonical previews in `data`, generated C reference covers all 1,578 exported functions across 13 pages, and Python guidance inventories all 18 adapted calls plus three generated helpers. All 12 real/prepared dataset showcases pass complete source, license, citation, preprocessing, and provenance checks, and a consolidated Known limitations page matches the public status vocabulary. The four-page visual-system pilot passes strict build and software-rendered desktop/mobile inspection. Isolated verification proves eight source screenshots current; 96 materially different recaptures require visual review and exact approval before any `data` update. | Review the visual pilot before broad rollout, review the 96 screenshot differences without touching `data`, review the tutorial voice, then complete the final RC3 inventory; reserve full-course chapters and asset freeze for RC4. |
