...
| Tutorial API and pilot | First-result evidence spike commit `0be0fe350` shares one renderer across live GLFW and deterministic offscreen Canvas paths, produces a nonblank `800x600` capture, passes bounded live/offscreen Vulkan validation, and proves borrowed-frame invariants. It exposed copied Canvas GPU setup, actual RGBA/BGRA frame-format resolution, per-frame wrapper allocation, raw dynamic-state calls, and frame-slot generation semantics. The automated macOS live-resize test remains an explicit skip because the requested external resize was not delivered. | Implement and prove the four general first-result API outcomes, close the live-resize and installed-consumer gates, then consolidate native shaders on `glslc`, move runtime shaderc into a focused thread-safe module, land the typed diagnostics API, package and validate the provider, and publish the three-chapter pilot before RC3. |
