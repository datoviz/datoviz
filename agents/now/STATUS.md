...
Status: active post-RC2 work toward RC3, then RC4 and final v0.4.0. Updated: 2026-08-18.
...
The optional Qt bridge and local Apple Silicon split-package proof are complete. The Vulkan-enabled Qt change is merged upstream, but the required RC3 provider lane remains externally blocked on publication of Qt 6.11.1 build 2 for both macOS architectures, followed by compatible PyQt and exact Datoviz provider artifacts.
...
| Qt/PyQt provider | Local Qt 6.11.1 build 2, PyQt6 6.11.0 build 3, split Datoviz packages, Vulkan instance, Cocoa surface, and hosted rendering proof are green; upstream Qt PR #406 is merged and its post-merge builds are running. | Verify Qt build 2 publication for both macOS architectures, rerun and publish compatible PyQt, then build and validate exact split Datoviz artifacts on supported hosted platforms. |
