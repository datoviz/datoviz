...
| WebGPU/WASM experimental path | WebGPU fixture runner works; the generic WASM scene ABI and split DRP2 packet path now register 77 scenarios and expose 76 browser-live gallery routes. The latest promotion covers coordinate/transform/View2D, Bezier/path-join, manual-camera/arcball, textured-mesh, reference-grid, and bounds-overlay examples. Scene/WASM picking proof passes, but browser smoke still checks pending-query state after `session.render()` has already drained it, so direct browser evidence for the ten latest routes remains open. | Follow the ordered coverage program in `spec/scene/integration/WASM_WEBGPU_PARITY_PLAN.md`: first repair picking output assertions and add route filtering, then prove the latest routes before graph/gizmo/label-probe, deferred basics, volume, techniques, and splat/point-cloud batches. |
