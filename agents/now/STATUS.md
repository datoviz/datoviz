...
| Label annotations and readouts | `Readout card first slice landed / rich block follow-up` | annotation/text realization tests, `examples/c/techniques/image_probe.c`, [`PINNED_READOUT_OVERLAY_CARD_PLAN.md`](PINNED_READOUT_OVERLAY_CARD_PLAN.md) | Add second internal card consumer or rich text-block raster/image lowering before public overlay API promotion. |
