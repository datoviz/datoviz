...
- Official default-on ImPlot/cimplot integration and the declarative docking refactor are a non-blocking pre-RC3 candidate under [GUI_IMPLOT_DOCKING_HANDOFF.md](GUI_IMPLOT_DOCKING_HANDOFF.md). PR #145 has merged, so the entry gate is open; each independently admissible slice must either land completely with affected package/platform proof before candidate freeze or defer intact until after RC3, and neither may delay required gates.
