...
| Tutorial API and pilot | Spike `0be0fe350` and API commits `f17e965f5`, `2e3627525`, `596f294af`, and `37e615627` close the four first-result API gaps with focused tests, bindings/reference refresh, validated live/offscreen runs, and standalone installed-package plus FetchContent consumer proof. Present frame-slot generation remains identity rather than recreation chronology. The automated macOS live-resize test remains an explicit skip because the requested external resize was not delivered. | Close the live-resize gate where the platform delivers it, then consolidate native shaders on `glslc`, move runtime shaderc into a focused thread-safe module, land the typed diagnostics API, package and validate the provider, and publish the three-chapter pilot before RC3. |
