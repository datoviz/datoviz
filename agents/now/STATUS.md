...
| 2 | Raw label-id GPU probe | First `dvz_labels()` raw integer probe slice is implemented for 2D labels fields and no longer depends on hidden RGBA image masks. | Broaden sparse/high-id pressure coverage and optimize the request readback path beyond full-texture copies. |
