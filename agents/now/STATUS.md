...
| WebGPU/WASM experimental path | WebGPU fixture runner works; the generic WASM scene ABI and split DRP2 packet path now register 89 scenarios and expose 84 browser-live gallery routes. Streaming DAQ has native and packet proof; its filtered browser run hit the known external headless instance-loss skip. SVG Tiger now has headed local-browser proof through an ignored cache-backed bundle; public promotion still awaits a separately approved prepared-data commit. | Obtain manual website confirmation for DAQ/galaxy/scatter/datetime/marker symbols, publish the approved SVG Tiger bundle in a separate data-submodule change, then continue the ordered coverage program with generic volume, techniques, and splat/point-cloud batches. |
