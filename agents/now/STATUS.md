...
Interaction pacing and presentation policy are implemented. Continuous interaction exposed stale-frame latency in ordinary FIFO rather than a scene-throughput defect. App-owned native windows prefer refresh-paced FIFO latest-ready, then mailbox, then a one-slot ordinary-FIFO fallback; explicit present-mode, FPS-cap, and frame-slot overrides remain available. Linux/X11 physical smokes found immediate and capped FIFO latest-ready very smooth while ordinary FIFO remained sluggish even with one slot; physical Windows frame-slot throughput measurements remain neutral.
