...
Status: active post-RC2 work toward RC3, then RC4 and final v0.4.0. Updated: 2026-07-25.
...
| Tutorial API and pilot | Spike `0be0fe350` and API commits `f17e965f5`, `2e3627525`, `596f294af`, and `37e615627` close the four first-result API gaps. Shader commits `f46ac75bd`, `3544a520a`, `06aad322d`, and `86bedc74c` consolidate native compilation and validation on one `glslc` helper, isolate the thread-safe runtime adapter, provide typed availability, diagnostics, file, profile, and ownership contracts, and move the triangle to restart-loaded external shaders. Pilot commits `1a748def3`, `3b567370a`, and `b5a5b989f` publish three compiled chapters from one canonical renderer, external shader variants, standalone installed-package CMake consumption, strict prose/source synchronization, and repeatable validated offscreen capture checks. Source-tree and locally installed macOS paths pass. Present frame-slot generation remains identity rather than recreation chronology. The automated macOS live-resize test remains an explicit skip because the requested external resize was not delivered. | Review and approve exact screenshot assets, complete official packaged-provider and Linux/Windows hosted proof, and close the live-resize gate where the platform delivers it. |
| Documentation and gallery freeze | Public RC2 documentation is deployed and the three-chapter Vulkan pilot is published without committed binary previews. Generated-reference completion, source-screenshot cache reconciliation, attribution, provenance, visual-pilot review, and final RC3 inventory remain. | Review the pilot voice and exact screenshot candidates, then freeze the non-tutorial surface in RC3; reserve the planned full-course chapters and asset freeze for RC4. |
