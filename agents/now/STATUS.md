...
| --- | --- | --- |
| WebGPU/WASM experimental path | WebGPU fixture runner works; generic WASM scene ABI emits split DRP2 packets for point, pixel, basic marker, basic segment, primitive, RGBA8 image, basic/textured mesh, basic sphere, panzoom, and a 3D sphere + textured mesh/arcball proof. Fresh 2026-06-05 browserless, native sphere-mode, and headless-browser proof is recorded in `examples/webgpu/COMPAT.md` and `docs/reference/webgpu-subset.md`. | Harden diagnostic ABI behavior and remove remaining demo shortcuts without expanding the RC subset. |
