...
| Release quality | Native, hosted, WebGPU, query, compute, documentation, gallery, and GPU-selection evidence exists. Validation-off sanitizer configuration is durable, the incremental source audit is complete through `input`, and the end-to-end remaining campaign is mapped on local branch `qa/rc3-source-audit`; see [QA_SOURCE_AUDIT.md](QA_SOURCE_AUDIT.md). | Review and approve the campaign mapping, then continue with `math` through the documented queue before the exact RC3 static-analysis, sanitizer where practical, Vulkan, long-loop, docs, gallery, example, source-archive, wheel, and installed-consumer gates; record limitations explicitly. |
