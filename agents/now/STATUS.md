...
| Release example proof | Partial. The broad `EXAMPLES_NOTES.md` pass has closed most source/gallery polish items; `showcases/surface_grid` and `features/bounds_overlay` are now ported. The remaining queue is the unresolved ledger in `EXAMPLES_NOTES.md` and the current handoff in `EXAMPLES_NOTES_IMPLEMENTATION_PROMPT.md`. | Finish the remaining runtime/readability fixes, complete scenario-helper/comments/builtin-shapes audits, and capture focused native evidence where the environment supports Vulkan. |
