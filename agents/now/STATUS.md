...
| Release example proof | Partial. The broad `EXAMPLES_NOTES.md` pass has closed most source/gallery polish items; the remaining queue is the unresolved ledger in `EXAMPLES_NOTES.md` and the current handoff in `EXAMPLES_NOTES_IMPLEMENTATION_PROMPT.md`. | Finish the remaining runtime/readability fixes, add `showcases/surface_grid` and `features/bounds_overlay`, complete scenario-helper/comments/builtin-shapes audits, and capture focused native evidence where the environment supports Vulkan. |
