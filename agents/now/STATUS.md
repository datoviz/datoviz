...
| Release example proof | Partial. Retained DATA-coordinate attachments and the choropleth migration away from resize-time geometry rebuild landed on 2026-06-05. One short public `examples/c/features/` source example now exists for the planned v0.4 feature queue. CPU/build proof passed; native choropleth visual smoke is blocked in this shell by Vulkan instance creation failure. | Compact native + WebGPU proof set and capture evidence for the new feature examples. Capture choropleth resize/equal-aspect visual proof in a Vulkan-capable environment. |
