...
| WebGPU/WASM experimental path | WebGPU fixture runner works; the generic WASM scene ABI and split DRP2 packet path now register 90 scenarios and expose 85 browser-live gallery routes. The point-cloud scenario has native capture and a deterministic 500k-point WASM packet proof; its local filtered browser route reaches the known external headless instance-loss skip. SVG Tiger has headed browser proof and an approved committed prepared-data bundle attributed to Nicolas P. Rougier's Glumpy example gallery. | Obtain manual website confirmation for the recent live routes, then continue generic volume and rendering techniques; public point-cloud promotion awaits a redistributable prepared bundle. |
