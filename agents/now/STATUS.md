...
| --- | --- | --- |
| WebGPU/WASM experimental path | WebGPU fixture runner works; generic WASM scene ABI emits split DRP2 packets for buffer-backed point/pixel positions, basic marker, segment/path with cap/join controls, primitive, RGBA8 image, low-level atlas glyph, semantic bitmap text, basic/textured/material mesh, basic sphere, panzoom, and a 3D sphere + textured mesh/arcball proof. Fresh 2026-06-05 browserless, native glyph/path/sphere/material/stroke-style/buffer-attr/text, and headless-browser proof is recorded in `examples/webgpu/COMPAT.md` and `docs/reference/webgpu-subset.md`. | Harden diagnostic ABI behavior and remove remaining demo shortcuts without expanding the RC subset. |
