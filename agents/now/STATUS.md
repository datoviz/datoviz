...
| Documentation and gallery | Generated reference, Python guidance, attribution, known limitations, gallery tooling, regenerated candidates, canonical screenshots, and the visual pilot are complete. Focused successor PR #136 implements the original author's final shader-tool embedding feedback and is clean, mergeable, and green on Linux, macOS, and Windows at `f3b47dbf5`. | Review the visual pilot and rewritten course voice; approve exact animation/card publication if desired; obtain contributor confirmation on PR #136, merge it with explicit approval, close PR #132 as superseded with separately approved public text, then draft RC3 notes and evidence after artifact scope freezes. |
