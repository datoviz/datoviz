...
| WebGPU/WASM experimental path | WebGPU fixture runner works; the generic WASM scene ABI and split DRP2 packet path now register 89 scenarios and expose 85 browser-live gallery routes. Streaming DAQ has native and packet proof; its filtered browser run hit the known external headless instance-loss skip. SVG Tiger has headed browser proof and an approved committed prepared-data bundle attributed to Nicolas P. Rougier's Glumpy example gallery. | Obtain manual website confirmation for DAQ/galaxy/scatter/datetime/marker symbols/SVG Tiger, then continue the ordered coverage program with generic volume, techniques, and splat/point-cloud batches. |
