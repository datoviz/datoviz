...
| Tutorial API and pilot | Spike `0be0fe350` and API commits `f17e965f5`, `2e3627525`, `596f294af`, and `37e615627` close the four first-result API gaps. Shader commits `f46ac75bd` and `3544a520a` consolidate native compilation and validation on one `glslc` helper and move runtime shaderc into a focused thread-safe module with enabled, disabled, concurrent-first-use, missing-provider, incompatible-provider, and packaged-provider proof. Present frame-slot generation remains identity rather than recreation chronology. The automated macOS live-resize test remains an explicit skip because the requested external resize was not delivered. | Land the typed shader availability, diagnostics, file, profile, and ownership API; package and validate the provider; close the live-resize gate where the platform delivers it; and publish the three-chapter pilot before RC3. |
