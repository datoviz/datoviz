...
Status: active post-RC2 work toward RC3, then RC4 and final v0.4.0. Local QA, course preparation, embedding portability, and gallery state updated: 2026-09-18. Published release and artifact state retain their earlier evidence; RC3 is not frozen.
...
RC3 implementation is integrated. The automated course review and [candidate preparation](RELEASE.md#rc3-preparation-before-maintainer-review-2026-09-08) are complete; maintainer review, candidate freeze, and exact artifact/platform proof remain. Completed local validation is not release approval. Use the lane records below rather than replaying completed campaigns.
...
| Fractal allocation hardening and prose | Code head `b67d0a95d`: [campaign and follow-ups](FRACTAL_CODE_HARDENING.md). Buffer growth preserves old resources on failure; constructor, Canvas readback, descriptor, image, and late import failures are covered. All 1,195 display-enabled native tests, four new llvmpipe sanitizer regressions, source Release/install consumers, a rebuilt local manylinux wheel with installed rendering/course checks, bindings/specification/docs, and bounded cppcheck pass. The NVIDIA leak reproduces in a standalone Vulkan client without Datoviz; Kvazaar differential checks pass, and the unused msdf exporter warning is suppressed only in that GCC source compilation. Vendor revisions remain unchanged; exact RC3 artifacts remain separate. |
...
The protected data branch remains `datoviz/data:v0.4-dev`, with current gallery tip `6a9091c10e08e477af5b07496cac602aefc7c999`; parent `9b0884f507f76f5a982db85cd2f6e59f094512e1` published the canonical histogram and verified gitlink. Preserve unrelated historical `data:main`. Follow [reachability and commit rules](../rules/REPO_HYGIENE.md) and the [asset migration contract](../../spec/data/ASSET_ARCHITECTURE.md); migration is not current RC3 work.
...
| Documentation and gallery | Generated reference, Python guidance, attribution, known limitations, gallery tooling, canonical screenshots, and the visual pilot are complete. The release guidance now separates RC3 from RC4, all 107 reviewed stills pass full-pixel validation, the nine invalidated still-cache records verify against canonical images, all 38 animation caches are current, all 29 video cards and posters fit budget, and experimental exclusions are explicit. The canonical histogram includes C, Python, and live WebGPU routes. PR #157 merged the refreshed embedding-portability work with green hosted checks; PR #136 is merged and superseded, and original PR #132 is closed as superseded. | Review the visual pilot, rewritten course voice, and exact animation/card candidates; approve publication if desired; review the local RC3 notes draft and finalize it after artifact scope freezes. |
| Distribution | RC2 wheel and package-index campaigns are complete; reusable source, wheel, conda, and vcpkg tooling exists. The checkout-backed Windows `x64-windows` overlay and standalone Debug/Release CMake consumers pass on the physical MSVC machine. Current local source packaging, licenses, installed consumers, and isolated diagnostic-wheel payload checks pass. The mimalloc dependency (submodule, CMake wiring, allocator backend, packaging references) was removed: its macOS `MI_OVERRIDE` zone/interpose default caused a `pointer being freed was not allocated` crash ([issue #155](https://github.com/datoviz/datoviz/issues/155)), it had already forced mimalloc off in macOS wheel CI once before, and no benchmark in this repo showed it improving Datoviz performance. | Validate the exact final RC3 Release source and six-wheel artifacts in supported builders, release-source vcpkg URL and SHA512, base conda layouts, third-party notices, and checksum/signing decisions. |
...
Hosted Linux and Windows exact-artifact proof is mandatory for RC3. The maintainer now has Linux, macOS, and Windows machines available; access, architecture, and desktop-session details remain to be confirmed for the prepared validation commands. Final v0.4.0 requires physical proof or an explicit maintainer-approved exception; no unavailable or unattended result is promoted to a pass.
