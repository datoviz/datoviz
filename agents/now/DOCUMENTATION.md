...
Build-local animated media is current under the implemented policy: all 38 animations regenerated at canonical `1280x720`, the comparison report selects 29 MP4 cards and nine animated WebPs, all poster and card variants remain within budget, and the full freshness gate passes. On 2026-07-30, isolated source-screenshot verification recaptured all 104 reviewed PNGs without touching `data`: four of the eight selected candidates were byte-identical, four stayed within the strict pixel-equivalence tolerance, and the checker now reports `ok=8, uncached=96`. The 96 materially different screenshots still require visual review and exact approval before any replacement; no PNG is missing or dimensionally invalid, and the deployed gallery remains present.
...
- Source-screenshot reconciliation revalidated eight exact/pixel-equivalent candidates under the finalized fingerprint and left 96 materially different recaptures uncached. Review the isolated difference set and obtain exact approval before replacing or staging any canonical PNG or the `data` gitlink.
