...
- The designated Linux reference workflow produced two byte-identical 104-image capture sets plus provenance and an old/new/diff review index. The maintainer reviewed and explicitly approved all 54 byte-different candidates; the canonical PNGs, committed promotion evidence, `data` commit, and parent gitlink are current. Keep future replacements behind the same Linux capture, repeatability, review, and exact-approval boundary.
